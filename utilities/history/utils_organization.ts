...
export const Transform_Organization = async (Organization: OrganizationDB, SongCol: Collection<SongDB>,
    ...
    const songs_in: SongDB[] = await SongCol.find({talk_about: Organization._id}).toArray();
    const album_in: AlbumDB[] = await AlbCol.find({talk_about: Organization._id}).toArray();
    const people_in: PersonDB[] = await PeoCol.find({_id: {$in: Organization.distinguished_members}}).toArray();
